...
for trial in total_trials:
    ...
    while i==0:
        ...
        print('Iteration number : ', iterator_count)
        w = w_b[0:30]
        b = w_b[30]
        s = s_f[0:30]
        f = s_f[30]
        ...
        """
        The first set of for loops after while loop calculates gradient at w_b for the check of
        convergence criteria which depends on the gradient of the function at the 'actual estimate'
        
        The second set of for loops calculates the gradient for Nesterov at s_f  (z in Nesterov)
        
        Notice that grad_b_1 and grad_w_1 are the same for grad_w_b and grad_s_f
    ...
    The stopping criteria in Nesterov is obtained via gradients and functional 
    evaluations at the actual estimate points w_b and not s_f which is just the
    interim z vector of Nesterov Algorithm
    ...
    """
    Rest portion of the code exactly same as before
